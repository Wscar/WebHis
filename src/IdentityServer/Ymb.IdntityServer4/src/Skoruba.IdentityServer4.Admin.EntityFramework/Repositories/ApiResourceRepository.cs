...
namespace Skoruba.IdentityServer4.Admin.EntityFramework.Repositories
{
        ...
        where TDbContext : DbContext, IAdminConfigurationDbContext
    {
        ...
        public virtual async Task<PagedList<ApiScope>> GetApiScopesAsync(int apiScopeId, int page = 1, int pageSize = 10)
        {
            var pagedList = new PagedList<ApiScope>();

            var apiScopes = await DbContext.ApiScopes
               
                .Where(x => x.Id == apiScopeId).PageBy(x => x.Name, page, pageSize).ToListAsync();

            pagedList.Data.AddRange(apiScopes);
            pagedList.TotalCount = await DbContext.ApiScopes.Where(x => x.Id == apiScopeId).CountAsync();
            pagedList.PageSize = pageSize;
